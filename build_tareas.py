...
# 1. Sidebar link
sidebar_link = '''                    <a href="#" class="menu-item" onclick="switchTab('ordenes')" id="menu-ordenes">
                        <svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                        Órdenes de Trabajo
                    </a>
                    <a href="#" class="menu-item" onclick="switchTab('tareas')" id="menu-tareas">
                        <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 11l3 3L22 4"></path><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path></svg>
                        Tareas
                    </a>'''
c = re.sub(r'<a href="#" class="menu-item" onclick="switchTab\(\'ordenes\'\)" id="menu-ordenes">[\s\S]*?Órdenes de Trabajo\s*</a>', sidebar_link, c)


# 2. allowedModules
c = c.replace("allowedModules = ['dashboard', 'ordenes', 'calendario', 'mapa', 'fichajes', 'jornales', 'database', 'colaboradores', 'clientes', 'materiales', 'manodeobra', 'presupuesto', 'configuracion'];", "allowedModules = ['dashboard', 'ordenes', 'tareas', 'calendario', 'mapa', 'fichajes', 'jornales', 'database', 'colaboradores', 'clientes', 'materiales', 'manodeobra', 'presupuesto', 'configuracion'];")
c = c.replace("allowedModules = ['dashboard', 'ordenes', 'fichajes'];", "allowedModules = ['dashboard', 'ordenes', 'tareas', 'fichajes'];")

# 3. Global state
c = c.replace("let planillas = [];", "let planillas = [];\n        let tareas = [];")
c = c.replace("window.localHistoricalPlanillas = [];", "window.localHistoricalPlanillas = [];\n        window.tareas = [];")
c = c.replace("fichajes = JSON.parse(localStorage.getItem('electrofrio_fichajes'));", "fichajes = JSON.parse(localStorage.getItem('electrofrio_fichajes'));\n            if (localStorage.getItem('electrofrio_tareas')) tareas = JSON.parse(localStorage.getItem('electrofrio_tareas')); window.tareas = tareas;")

# 4. Module HTML (Inject before Ordenes)
tareas_html = '''
        <!-- ======================= -->
        <!-- MÓDULO DE TAREAS        -->
        <!-- ======================= -->
        <section id="tareas" class="module hidden">
            ...
            <div id="tareas-admin-container" class="role-admin-only" style="margin-top:20px;">
            ...
            <div id="tareas-collab-container" class="role-collab-only" style="margin-top:20px; display: grid; gap: 15px; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));">
...
'''
c = c.replace('// --- FUNCIONES Y EVENTOS ---', js_logic + '\n        // --- FUNCIONES Y EVENTOS ---')

c = c.replace('renderPresupuestoTable();', 'renderPresupuestoTable(); renderTareas();')
...
'''
c = c.replace('function initializeFirebaseListeners() {', 'function initializeFirebaseListeners() {' + snapshot_logic)
